...
const ensureQualityTable = async () => {
    ...
    const countRes = await (0, db_1.query)('SELECT COUNT(*) FROM quality_inspections');
...
router.get('/stats', async (req, res) => {
    try {
        const passRes = await (0, db_1.query)("SELECT COUNT(*) FROM quality_inspections WHERE status = 'Pass'");
        const failRes = await (0, db_1.query)("SELECT COUNT(*) FROM quality_inspections WHERE status = 'Fail'");
